...
df.mi <- read_csv("Model_identifiability_em.csv", col_names = T)

df.mi <- df.mi %>%
  mutate(Simulated = factor(Simulated,
                            levels = c(1,2,3,4),
                            labels = c(paste0("1","<i>\u03B1</i>","1","<i>\u03B2</i>"), # rename models with alpha and beta symbols
                                       paste0("3","<i>\u03B1</i>","1","<i>\u03B2</i>"),
                                       paste0("2","<i>\u03B1</i>","1","<i>\u03B2</i>"),
                                       paste0("3","<i>\u03B1</i>","3","<i>\u03B2</i>")))) %>%
  mutate(Estimated = factor(Estimated,
                            levels = c(1,2,3,4),
                            labels = c(paste0("1","<i>\u03B1</i>","1","<i>\u03B2</i>"),
                                       paste0("3","<i>\u03B1</i>","1","<i>\u03B2</i>"),
                                       paste0("2","<i>\u03B1</i>","1","<i>\u03B2</i>"),
                                       paste0("3","<i>\u03B1</i>","3","<i>\u03B2</i>"))))
...
df.pr <- read_csv("Parameter_recovery_em.csv", col_names = T)
...
h.plot <- ggplot(data = df.pr, aes(x=Simulated, y=Recovered, fill=HCorr)) + 
  ...
  scale_x_continuous(breaks = c(1,2,3,4),
                     labels = c(paste0("<i>\u03B1</i>","<sub>self</sub>"), # rename models with alpha and beta symbols
                                paste0("<i>\u03B1</i>","<sub>other</sub>"),
                                paste0("<i>\u03B1</i>","<sub>no one</sub>"),
                                paste0("<i>\u03B2</i>"))) +
  scale_y_continuous(breaks = c(1,2,3,4),
                     labels = c(paste0("<i>\u03B1</i>","<sub>self</sub>"),
                                paste0("<i>\u03B1</i>","<sub>other</sub>"),
                                paste0("<i>\u03B1</i>","<sub>no one</sub>"),
                                paste0("<i>\u03B2</i>"))) +
  theme_classic() + 
...
df.fit <- read_csv("PL_model_fit_statistics.csv", col_names = T)
...
df.tbt <- read_csv("Task_data_trial_by_trial.csv", col_names = T) 
...
df.tav <- read_csv("Task_data_participant_averages.csv", col_names = T) 
...
row.names(performstats) <- c("Intercept", "Trial number", "Agent (other vs self)", "Agent (no one vs self)", "Group (older vs young)", "Trial number * agent (other vs self)", "Trial number * agent (no one vs self)", "Trial number * group", "Group * agent (other vs self)", "Group * agent (no one vs self)", "Trial number * group * agent (other vs self)", "Trial number * group * agent (no one vs self)")
kable(performstats,
      align = rep('c',6),
...
df.full <- read_csv("PL_152_combined_data_share.csv", col_names = T) 
...
row.names(alphastats) <- c("Intercept", "Agent (other vs self)", "Agent (no one vs self)", "Group (older vs young)", "Group * agent (other vs self)", "Group * agent (no one vs self)")
kable(alphastats,
      align = rep('c',6),
...
fit.directeffect <- rlm(`other-self_alph` ~ group, df.med)
m.directeffect <- model_parameters(fit.directeffect, standardize = "refit")
# print(summary(fit.directeffect))
...
standard.model <- tibble(Path = c("a", "b", "c", "c'"), 
                Coefficient = c(m.mediator$Coefficient[nrow(m.mediator)], m.dv$Coefficient[nrow(m.dv)], m.directeffect$Coefficient[nrow(m.directeffect)], m.dv$Coefficient[nrow(m.dv)-1]), 
                CI_low = c(m.mediator$CI_low[nrow(m.mediator)], m.dv$CI_low[nrow(m.dv)], m.directeffect$CI_low[nrow(m.directeffect)], m.dv$CI_low[nrow(m.dv)-1]), 
                CI_high = c(m.mediator$CI_high[nrow(m.mediator)], m.dv$CI_high[nrow(m.dv)], m.directeffect$CI_high[nrow(m.directeffect)], m.dv$CI_high[nrow(m.dv)-1]), 
                t = c(m.mediator$t[nrow(m.mediator)], m.dv$t[nrow(m.dv)], m.directeffect$t[nrow(m.directeffect)], m.dv$t[nrow(m.dv)-1]), 
                df_error = c(m.mediator$df_error[nrow(m.mediator)], m.dv$df_error[nrow(m.dv)], m.directeffect$df_error[nrow(m.directeffect)], m.dv$df_error[nrow(m.dv)-1]), 
                p = c(m.mediator$p[nrow(m.mediator)], m.dv$p[nrow(m.dv)], m.directeffect$p[nrow(m.directeffect)], m.dv$p[nrow(m.dv)-1]))

kable(standard.model,
      align = c('l', rep('c',6)),
      caption = "Standard mediation model" ) %>%
  kable_styling() 

moderated.model <- tibble(Path = c("a", "b young", "b older", "c", "c'"), 
                Coefficient = c(m.mediator$Coefficient[nrow(m.mediator)], m.young$Coefficient[nrow(m.young)], m.old$Coefficient[nrow(m.old)], m.directeffect$Coefficient[nrow(m.directeffect)], m.dv.mod$Coefficient[nrow(m.dv.mod)-2]),
                CI_low = c(m.mediator$CI_low[nrow(m.mediator)], m.young$CI_low[nrow(m.young)], m.old$CI_low[nrow(m.old)], m.directeffect$CI_low[nrow(m.directeffect)], m.dv.mod$CI_low[nrow(m.dv.mod)-2]), 
                CI_high = c(m.mediator$CI_high[nrow(m.mediator)], m.young$CI_high[nrow(m.young)], m.old$CI_high[nrow(m.old)], m.directeffect$CI_high[nrow(m.directeffect)], m.dv.mod$CI_high[nrow(m.dv.mod)-2]), 
                t = c(m.mediator$t[nrow(m.mediator)], m.young$t[nrow(m.young)], m.old$t[nrow(m.old)], m.directeffect$t[nrow(m.directeffect)], m.dv.mod$t[nrow(m.dv.mod)-2]), 
                df_error = c(m.mediator$df_error[nrow(m.mediator)], m.young$df_error[nrow(m.young)], m.old$df_error[nrow(m.old)], m.directeffect$df_error[nrow(m.directeffect)], m.dv.mod$df_error[nrow(m.dv.mod)-2]), 
                p = c(m.mediator$p[nrow(m.mediator)], m.young$p[nrow(m.young)], m.old$p[nrow(m.old)], m.directeffect$p[nrow(m.directeffect)], m.dv.mod$p[nrow(m.dv.mod)-2]))

kable(moderated.model,
      align = c('l', rep('c',6)),
...
plt <- ggplot(plt.data, aes(x=group, y=bootMed, alpha = group)) +
  ...
  labs(x= NULL,
       y = "<i>\u03B1</i><sub>other</sub> - <i>\u03B1</i><sub>self</sub><br>") +
  ...
  theme(axis.text.x=element_text(size = axtitle, hjust = c(0.7,0.3), colour = "black"), 
        ...
        axis.title.y=element_markdown(size = axtitle)) +
  theme(legend.position = 'none') + 
  annotation_custom(grob = textGrob("less prosocial", x=-0.55,  y=0.18, rot = 90,
                                    gp=gpar(fontsize=14, fontface="italic"))) +
  annotation_custom(grob = textGrob("more prosocial", x=-0.55,  y=0.8, rot = 90,
                                    gp=gpar(fontsize=14, fontface="italic")))
...
plt <- ggplot(plt.data, aes(x=group, y=bootMed, alpha = group)) +
  ...
  scale_y_continuous(name = 'Temperature parameter (<i>\u03B2</i>)', breaks = c(0.2,0.4,0.6,0.8,1,1.2), expand = expand_scale(mult = c(0, .1))) +
  scale_fill_manual(values = bar1col, labels = c('Young', 'Older')) +
  scale_color_manual(values = inddata1cols, labels = c('Young', 'Older')) +
  scale_alpha_manual(values = c(1, 0.33), labels = c('Young', 'Older')) +
  theme_classic() +
...
plot.other.ai.old <- ggplot(df.full.older, aes(x = `agent2-alpha`, y = SRP_affective_interpersonal)) + 
  ...
  scale_x_continuous(name = paste0("<i>\u03B1</i>","<sub>other</sub>"," (Older)"), breaks = c(0.02, 0.04, 0.06, 0.08, 0.1), labels = c('0.02', '0.04', '0.06', '0.08', '0.10')) +
...
plot.other.ai.young <- ggplot(df.full.young, aes(x = `agent2-alpha`, y = SRP_affective_interpersonal)) + 
  ...
  geom_smooth(aes(group = group, color = group, fill = group, linetype = group), method = 'lm', alpha = 0.3, size = 0.75, show.legend = FALSE) + 
  geom_smooth(aes(group = group, color = group, fill = group, linetype = group), method = 'lm', alpha = 0, size = 2) +
  scale_x_continuous(name = paste0("<i>\u03B1</i>","<sub>other</sub>"," (Young)"), breaks = c(0.02, 0.04, 0.06, 0.08, 0.1), labels = c('0.02', '0.04', '0.06', '0.08', '0.10')) +
  scale_y_continuous(name = "Psychopathic traits<br>
        <span style='font-size:14pt'>(Affective-interpersonal subscale)</span>", breaks = seq(10, 60, 10)) + 
  coord_cartesian(xlim=c(0.018, 0.102), ylim=c(10, 60)) + 
...
df.opt <- read_csv("Optimal_alpha.csv", col_names = T)
...
plot.alpha.corr.sim <- ggplot(df.opt, aes(x = Alpha, y = Choices)) + 
  ...
  scale_x_continuous(name = paste0("Learning rate (<i>\u03B1</i>)"), n.breaks = 5) +
...
plot.alpha.corr <- ggplot(df.l, aes(x = alpha, y = corr)) + 
  ...
  scale_x_continuous(name = paste0("Learning rate (<i>\u03B1</i>)")) + #, n.breaks = 7) +
  scale_y_continuous(name = "Percentage correct") +
...
# loads data
group.fit <- read_csv("Compare_fit_between_groups.csv", col_names = T) 
group.fit <- group.fit %>%
  mutate(group = factor(group, 
                        levels = c(1,2), 
                        labels = c("young", "old"))) 
...
row.names(alphastats.W) <- c("Intercept", "IQ", "Agent (other vs self)", "Agent (no one vs self)", "Group (older vs young)", "Group * agent (other vs self)", "Group * agent (no one vs self)")
kable(alphastats.W,
      align = rep('c',6),
...
kable(corr.table.s, 
      align = c(rep('l',2), rep('c',2)),
      caption = "Correlations between alphas and IQ" ) %>%
  kable_styling() 
...
df.mi <- df.mi %>%
  mutate(Simulated = factor(Simulated,
                            levels = c(paste0("1","<i>\u03B1</i>","1","<i>\u03B2</i>"), # remove html formatting
                                       paste0("3","<i>\u03B1</i>","1","<i>\u03B2</i>"),
                                       paste0("2","<i>\u03B1</i>","1","<i>\u03B2</i>"),
                                       paste0("3","<i>\u03B1</i>","3","<i>\u03B2</i>")),
...
df.pr <- df.pr %>%  
  mutate(Simulated = factor(Simulated,
                            levels = c(1,2,3,4),
                            labels = c(paste0("\u03B1","self"), # rename models with alpha and beta symbols
                                       paste0("\u03B1","other"),
                                       paste0("\u03B1","no one"),
                                       paste0("\u03B2")))) %>% 
  mutate(Recovered = factor(Recovered,
                            levels = c(1,2,3,4),
                            labels = c(paste0("\u03B1","self"),
                                       paste0("\u03B1","other"),
                                       paste0("\u03B1","no one"),
                                       paste0("\u03B2"))))
...
# Fig. 2e in correct format: df.opt

# Fig. 2f

df.2f <- df.l %>% select(c("ID", "agent", "alpha", "corr"))

# Fig. 3a 

df.3a <- df.3a %>% select(c("agent", "group", "n", "bootMed", "bootSE"))

# Fig. 3b

df.3b <- df.3b %>% select(c("group", "n", "bootMed", "bootSE"))

# 3c in correct format: df.3c

# Fig. 4a

df.full.older <- df.full.older %>% select(c("agent2-alpha", "SRP_affective_interpersonal")) %>% drop_na()

# Fig. 4b
  
df.full.young <- df.full.young %>% select(c("agent2-alpha", "SRP_affective_interpersonal")) %>% drop_na()

# Fig. 5a in correct format: standard.model

# Fig. 5b in correct format: moderated.model

# Fig. S1

df.s1 <- df.s1 %>% select(c("group", "n", "bootMed", "bootSE"))

# Table S3 in correct format: corr.table.r 

# Table S4

df.s4 <- tibble(agent = c("other", "other", "other-self", "other-self"), 
                correlation = rep(c("standard", "partial"), 2), 
                rs = c(corr.old.other.ai$r[1,2], pcorr.old.other.ai$estimate, corr.old.otherself.ai$r[1,2], pcorr.old.os.ai$estimate), 
                p = c(corr.old.other.ai$P[1,2], pcorr.old.other.ai$p.value, corr.old.otherself.ai$P[1,2], pcorr.old.os.ai$p.value))

# Table S5 in correct format: corr.table.ai

# Table S7 in correct format: corr.table.ai3

# Table S9

df.s9 <- bind_rows(corr.table.s, corr.table.ace)
df.s9$Group[is.na(df.s9$Group)] <- "Older"
df.s9$Subscale[is.na(df.s9$Subscale)] <- "IQ"

# Table S10

df.s10 <- tibble(agent = c(rep("other", 4), rep("other-self", 4)), 
                 correlation = rep(c("standard", rep("partial", 3)), 2), 
                 control = rep(c("NA", "IQ", "Memory", "Attention"),2),
                 rs = c(corr.old.other.ai$r[1,2], pcorr.old.other.ai.s$estimate, pcorr.old.other.ai.m$estimate, pcorr.old.other.ai.a$estimate,
                        corr.old.otherself.ai$r[1,2], pcorr.old.os.ai.s$estimate, pcorr.old.os.ai.m$estimate, pcorr.old.os.ai.a$estimate), 
                 p = c(corr.old.other.ai$P[1,2], pcorr.old.other.ai.s$p.value, pcorr.old.other.ai.m$p.value, pcorr.old.other.ai.a$p.value,
                       corr.old.otherself.ai$P[1,2], pcorr.old.os.ai.s$p.value, pcorr.old.os.ai.m$p.value, pcorr.old.os.ai.a$p.value))

source.data <- list(df.mi, df.pr, df.fit, df.opt, df.2f, df.3a, df.3b, df.3c, df.full.older, df.full.young, standard.model, moderated.model, df.s1, corr.table.r, df.s4, corr.table.ai, corr.table.ai3, df.s9, df.s10)
names(source.data) <- c("Fig2a-b", "Fig2c", "Fig2d", "Fig2e", "Fig2f", "Fig3a", "Fig3b", "Fig3c", "Fig4a", "Fig4b", "Fig5a", "Fig5b", "FigS1", "TableS3", "TableS4", "TableS5", "TableS7", "TableS9", "TableS10")
