...
elseif       strcmp(modelID,'ms_RWPL_SO_LR')
   if size(qin,2)~=get_npar(modelID), disp('ERROR'); keyboard; end
   qout     = [norm2beta(qin(:,1)) norm2alpha(qin(:,2)) norm2alpha(qin(:,3)) norm2alpha(qin(:,4))];
...
elseif  strcmp(modelID,'ms_RWPL_SON_LR_SON_beta')
   if size(qin,2)~=get_npar(modelID), disp('ERROR'); keyboard; end
   qout     = [norm2beta(qin(:,1)), norm2beta(qin(:,2)), norm2beta(qin(:,3)), norm2alpha(qin(:,4)), norm2alpha(qin(:,5)), norm2alpha(qin(:,6))];
